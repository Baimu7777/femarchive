...
本节是一套完整的日常发帖流程。即使第一次操作，也可以从头到尾依次完成。
...
终端打开后，不需要理解里面已经显示的路径。只要确认自己是在 VS Code 打开的文库网站项目中即可。

或者也可以在文件夹根目录鼠标右键→在终端打开
...
| --- | --- | --- |
| `title` | 页面标题 | 填写文章标题 |
| `description` | 首页卡片和搜索结果中的简介 | 填写一到两句话 |
| `slug` | 网页链接最后一段 | 填写一个没有重复的编号 |
| `draft` | 是否公开发布 | 正式发布写 `false`；草稿写 `true` |
| `comments` | 是否显示页面底部在线评论区 | 开启写 `true`；关闭写 `false` |
| `author` | 作者 | 填写作者名 |
| `categories` | 普通文章所属大类 | 从网站已有分类中选择 |
| `tags` | 更细的关键词 | 可以填写多个 |

其她属性不确定怎样填写时，可以暂时往下跳，查看 [第 4 部分：文章属性设置](#4-文章属性设置)。
...
插入图片、链接、引用、卡片或折叠内容时，可以跳到 [第 5 部分：按需查阅](#5-按需查阅markdown图片与短代码)。
