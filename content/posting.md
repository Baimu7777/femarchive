...
创建文章和启动本地预览时，需要在 PowerShell 中输入命令。

打开方法：

1. 打开文件夹根目录`/femarchive`
2. 鼠标右键，选择`在终端打开`
3. 按下回车键。
...
其中，`example`为该文件的命名，可以自己随意修改。

然后按下回车键。

命令运行后，`content/posts` 文件夹中会出现一个新的 `.md` 文件。
...
| 内容类型 | 创建命令 |
| --- | --- |
| 普通文章 | `hugo new content/posts/example.md` |
| 新闻文章 | `hugo new content/news/example.md` |
| 女性人物 | `hugo new content/figures/example.md` |
...
文件名只是电脑中的名称，可以随意设置。建议在终端中只输入数字和英文，如果需要修改为中文文件名，如`27女性的定义是什么.md`，可在生成文件后再进行重命名。拿不准文件命名方式的可以看看文件夹里别的文件都是怎么写的。网页地址是通过文章开头的 `slug` 单独设置的。
...
软件可以直接在下方表格中点击下载。Hugo使用的是148.1版本，如果要自己下载安装的话要注意版本统一：
...
如果提示“无法识别 hugo 命令”，通常是环境变量还没有生效。可以检查 D:\Hugo 里面是否有 hugo.exe，然后关闭终端重新打开，再试一次。
