...
如果这是你第一次参与网站维护，电脑中还没有网站文件夹，请先阅读 [第 2 部分：第一次使用前的准备](#2-第一次使用前的准备)。
...
1. 点击顶部菜单栏的 **Repository**。
2. 点击 **Open in Visual Studio Code**。
...
文件名只是电脑中的名称。网页地址可以通过文章开头的 `slug` 单独设置。
...
| 顺序 | 操作 |
| --- | --- |
| 1 | 打开 GitHub Desktop，Fetch；如有更新，再 Pull |
| 2 | 点击 Repository → Open in Visual Studio Code |
| 3 | 点击终端 → 新建终端 |
| 4 | 输入 `hugo new content/posts/文件名.md` |
| 5 | 填写文章属性和正文，按 `Ctrl + S` 保存 |
| 6 | 输入 `hugo server`，浏览器打开 `http://localhost:1313/` |
| 7 | 回到 GitHub Desktop，填写 Summary，点击 Commit |
| 8 | 点击 Push origin |
...
| 软件 | 用途 | 是否必须 |
| --- | --- | --- |
| GitHub Desktop | 用按钮完成同步、保存版本、上传和回退 | 必须 |
| VS Code | 打开网站文件夹、编辑文章、运行预览命令 | 必须 |
| Hugo Extended | 在电脑上创建文章文件并预览网站 | 必须 |
| Git | GitHub Desktop 已经自带 Git；只用 GitHub Desktop 时不需要单独安装 | 通常不需要另外安装 |
...
日常发帖并不需要先理解这一部分。遇到多人协作、版本冲突或误操作时，再回来查阅即可。
...
# 5. 按需查阅：Markdown、图片与短代码

这部分不需要从头读到尾。遇到具体需求时，直接找到对应小节即可。

## 5.1 可以先在 Notion 中写文章吗？
